...
class HilbertHandler(BaseHTTPRequestHandler):
    ...
    def get_available_models(self):
        ...
        try:
            with urllib.request.urlopen(self.llama_url("/models"), timeout=5) as response:
    ...
    def check_model(self):
        try:
            with urllib.request.urlopen(self.llama_url("/health"), timeout=2) as response:
...
def main():
    parser = argparse.ArgumentParser(description="LAN chat UI for the local llama.cpp server.")
    ...
    print(f"Using llama.cpp at http://{args.llama_host}:{args.llama_port}", flush=True)
