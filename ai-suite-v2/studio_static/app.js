...
const PIPELINES = [
  ...
  { id: "audio-ace-step", category: "Audio", name: "ACE-Step 1.5", mediaType: "audio", workflowId: "audio-ace-step-1-5" },
  { id: "audio-diffrhythm", category: "Audio", name: "DiffRhythm", mediaType: "audio", workflowId: "audio-diffrhythm" },
...
async function refreshLlmEndpoints() {
  const panel = $("#llmEndpoints");
  if (!panel) return;
  try {
    const response = await fetch("/api/llm-endpoints");
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || "LLM endpoint check failed");
    state.llmEndpoints = payload.endpoints || [];
    state.llmNetworkConfigured = Boolean(payload.configured_network);
    renderLlmEndpoints();
  } catch (error) {
    panel.innerHTML = `<div class="runtime-empty">${escapeHtml(error.message)}</div>`;
  }
}
...
function renderLlmEndpoints() {
  ...
  if (!panel) return;
  panel.innerHTML = "";
  if (!state.llmEndpoints.length) {
    panel.innerHTML = `<div class="runtime-empty">No LLM endpoints configured.</div>`;
    return;
  }
  for (const endpoint of state.llmEndpoints) {
    const item = document.createElement("article");
    item.className = `runtime-item ${endpoint.online ? "online" : ""}`;
    const modelList = (endpoint.models || []).slice(0, 3).join(", ");
    item.innerHTML = `
      <span class="runtime-dot"></span>
      <div class="runtime-copy">
        <strong>${escapeHtml(endpoint.name)}</strong>
        <small>${escapeHtml(endpoint.model || "model auto")}</small>
        <small title="${escapeHtml(endpoint.url || "")}">${escapeHtml(endpoint.status || "unchecked")}</small>
        ${modelList ? `<small title="${escapeHtml((endpoint.models || []).join(", "))}">${escapeHtml(modelList)}</small>` : ""}
      </div>
    `;
    panel.append(item);
  }
  if (!state.llmNetworkConfigured) {
    const hint = document.createElement("div");
    hint.className = "runtime-empty";
    hint.textContent = "Add network servers in LLM_ENDPOINTS to try them before the sidecar.";
    panel.append(hint);
  }
}
...
function mediaMarkup(item, thumb) {
  ...
  if (item.type === "audio") {
    return `<audio src="${item.url}" ${thumb ? "" : "controls"}></audio>`;
  }
  if (thumb) {
...
$("#refreshLlmEndpoints")?.addEventListener("click", refreshLlmEndpoints);
